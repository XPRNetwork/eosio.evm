...
get_root_of_unity(const size_t n)
{
    ...
    if (n != (1u << logn)) return eosio::check(false, "libff::get_root_of_unity: expected n == (1u << logn)");
    if (logn > FieldT::s) return eosio::check(false, "libff::get_root_of_unity: expected logn <= FieldT::s");
